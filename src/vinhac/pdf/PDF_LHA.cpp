...
LHAPDF::PDFSetInfo VINHAC::PDF_LHA::initPDFByName(const string &name, LHAPDF::SetType type,
		int memset) {
#ifdef DEBUG_FLAG
	...
	if(type==0){
		...
		throw VinhacException("Unrecognized LHAPDF::SetType = "+type);
	}
	try{
	  return LHAPDF::getPDFSetInfo(name+settype,memset);
	} catch(...){
		std::ostringstream os;
		os<<"Error while getting PDFInfo, file = "
		<<name
		<<settype
		<<", memset = "
		<<memset
