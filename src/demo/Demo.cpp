...
#include "../vinhac/demo/DemoAnalysisHandler.h"
...
int main(int argc, char* argv[]) {
	...
	for(i=0; i<eventsNb; i++){
		...
		if(fmod(i+1,100000)==0){
			generator.makeSnapshot(outputPath,i+1);
	...
	kanwa6.cd(1);
	demoAnalysisHandler.getWPT().Draw();

	TCanvas kanwa7("kanwa7","Histograms7",5,5,1024,768);
	kanwa7.cd(1);
	demoAnalysisHandler.getPhiHist().Draw();

	TCanvas kanwa8("kanwa8","Histograms8",5,5,1024,768);
	kanwa8.cd(1);
	demoAnalysisHandler.getCosThetaWLHist().Draw();

	TCanvas kanwa9("kanwa9","Histograms9",5,5,1024,768);
	kanwa9.cd(1);
	demoAnalysisHandler.getPhiWLHist().Draw();
