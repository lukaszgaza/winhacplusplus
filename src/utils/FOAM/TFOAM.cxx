...
// Constructor
  ...
  m_Chat     = 2;                // Chat=0,1,2 chat level in output, Chat=1 normal level
...
//  Basic initialization, create "FOAM of cells"
  ...
  if(m_TotDim==0) StopM(std::string("TFOAM::Initialize: zero dimension not alowed"));
  ...
  if(m_Rvec==NULL)  StopM(std::string("TFOAM::Initialize: Cannot initialize buffer m_Rvec"));

  if(m_nDim>0){
    m_Lambda = new double[m_nDim];   // sum<1 for internal parametrization of the simplex
    if(m_Lambda==NULL)  StopM(std::string("TFOAM::Initialize: Cannot initialize buffer m_Lambda"));
  }
  if(m_kDim>0){
    m_Alpha = new double[m_kDim];    // sum<1 for internal parametrization of the simplex
    if(m_Alpha==NULL)  StopM(std::string("TFOAM::Initialize: Cannot initialize buffer m_Alpha"));
  }
  m_MCvect = new double[m_TotDim]; // vector generated in the MC run
  if(m_MCvect==NULL)  StopM(std::string("TFOAM::Initialize: Cannot initialize buffer m_MCvect"));
  ...
  if(m_HistEdg==NULL) StopM(std::string("TFOAM::Initializes: Cannot allocate m_HistEdg"));
#endif
...
//
  ...
  if(m_nDim==0) StopM(std::string("TFOAM::InitVertices: m_nDim=0"));
  switch(m_OptOrd){
  case 0:
    ///////////////////////////////////////////////////////////////////////
    ...
    if(nDivi < 1 ) StopM(std::string("TFOAM::Initialize: too big m_nDim or too small m_nCells"));
    ...
    if(m_VerX==NULL) StopM(std::string("TFOAM::InitVertices: Cannot initialize buffer for vertices"));
    /////////////////////////////////////////////////////////
    ...
    if(iVe != NoVert ) StopM(std::string("TFOAM::InitVertices: Something wrong with sum ove partitions"));
    break;
  case 1:
    ///////////////////////////////////////////////////////////////////////
    ...
    break;
  default:
    StopM(std::string("STOP in TFOAM::InitVertices - wrong m_OptOrd = "));
  }//switch
  m_LastVe = NoVert-1;
...
// Initialize "root part" of the FOAM of cells
  ...
  if(m_Cells==NULL) StopM(std::string("TFOAM::InitCells: Cannot initialize CELLS"));

  // define initial values of Posi Size vectors, if h-cubic is present
  if(m_kDim>0){
     Posi = new TFVECT(m_kDim);
     Size = new TFVECT(m_kDim);
     if(Posi==NULL||Size==NULL) StopM(std::string("TFOAM::nitCells: Cannot allocate Posi or Size"));
     (*Posi)=0.0;
     (*Size)=1.0;
  }
  ...
  if(m_nDim==0 && m_kDim>0){
	...
	Vertices[k] = k;          // FILL in pointers to vertices
      ...
      break;
    default:
      StopM(std::string("STOP in TFOAM::InitCells - wrong m_OptOrd"));
    }//switch(OptOrd)
  }//if(nDim>0)
...
//   already defined prior to calling this routine.
  ...
  if(m_nDim>0){
    ...
    if(VolPart==NULL)   StopM(std::string("TFOAM::Initialize: Cannot initialize buffer VolPart"));
  ...
  case 2:                       // WTMAX  REDUCTION
    ...
    break;
  default:
    StopM(std::string("STOP in TFOAM::Explore - wrong m_OptDrive = "));
  }//switch
  //=================================================================================
...
// using results of the MC exploration run within the cell stored in m_HistEdg
  ...
  if( (kBest >= m_nProj) || (kBest<0) ) StopM(std::string("TFOAM::Varedu: something wrong with kBest"));
}          //TFOAM::Varedu
...
// using results of the MC exploration run within the cell stored in m_HistEdg
  ...
  if(Bins==NULL)    StopM(std::string("TFOAM::Carver: Cannot initialize buffer Bins"));
  ...
  if( (kBest >= m_nProj) || (kBest<0) ) StopM(std::string("TFOAM::Carver: something wrong with kBest"));
  delete [] Bins;
}          //TFOAM::Carver
...
//  Grow new cells by division
  ...
  while ( (m_LastCe+2) < m_nCells ){  // this condition also checked inside Divide
    ...
    case 0:
      ...
      if( (iCell<0) || (iCell>m_LastCe) ) StopM(std::string("TFOAM::Grow: wrong iCell"));
...
// to their volume estimates.
   ...
   if( (m_nDim>1)&&(m_OptOrd==0)){
     ...
     if( (fabs(TotDriv -m_Cells[0]->GetDriv() ) > 1.0e-5*TotDriv) )
       StopM(std::string("STOP in TFOAM::Peek -> something wrong with total driver integral"));
     if( (fabs(TotIntg -m_Cells[0]->GetIntg() ) > 1.0e-5*TotDriv) )
       StopM(std::string("STOP in TFOAM::Peek -> something wrong with total true integral"));
     ...
     if (iDau<1 ) StopM(std::string("TFOAM::PeekRan: something went wrong, iDau<1 !!!!"));
...
//  Return Code RC=-1 of buffer limit is reached,  m_LastCe=m_nBuf
  ...
  if(m_LastCe+1 >= m_nCells) StopM(std::string("TFOAM::Divide: buffer limit is reached, m_LastCe=m_nBuf"));
  ...
  if(m_nDim>0 && m_OptVert==0){
    ...
    if(kVer1==NULL||kVer2==NULL) StopM(std::string("TFOAM::Divide: Cannot initialize buffer kVer"));
    for(jv=0; jv<m_nDim+1; jv++){
  ...
  Xdiv  = Cell->GetXdiv();
  kBest = Cell->GetBest();
  if( kBest<0 || kBest>=m_nProj ) StopM(std::string("TFOAM::Divide: wrong kBest"));
  ...
  if( ( m_P0Si <= kBest ) && ( kBest < m_P0Si+nProj0 ) ){
    ...
    OldVe1 =(*Cell)[Old1];
    OldVe2 =(*Cell)[Old2];
    if (m_LastVe+1>m_vMax) StopM(std::string("STOP in TFOAM::Divide: too short list of vertices "));
    ...
    if( m_OptMCell==0 ) {
      ...
      if(kDiv<0||kDiv>=m_kDim) StopM(std::string("Divide: wrong kDiv"));  // kDiv is form zero to m_kDim-1 !!!
...
void TFOAM::MakeActiveList(void){
  ...
  if( m_CellsAct==NULL || m_PrimAcu==NULL ) StopM(std::string("MakeActiveList - Cant allocate m_CellsAct or m_PrimAcu"));
...
// This should be called before Initialize, after setting nDim and kDim
  if(m_kDim==0) StopM(std::string("TFOAM::SetInhiDiv: m_kDim=0"));
  if(m_InhiDiv == NULL){
    m_InhiDiv = new int[ m_kDim ];
  ...
  if( ( 0<=iDim) && (iDim<m_kDim)){
    ...
    StopM(std::string("TFOAM::SetInhiDiv: wrong iDim"));
}//SetInhiDiv
...
// 0<=iDim<_kDim is in H-cubic subspace!!!
  if(m_kDim<=0)  StopM(std::string("TFOAM::SetXdivPRD: m_kDim=0"));
  if(   len<1 )  StopM(std::string("TFOAM::SetXdivPRD: len<1"));
  ...
  if( ( 0<=iDim) && (iDim<m_kDim)){
    ...
    if( m_XdivPRD[iDim] != NULL)
      StopM(std::string("TFOAM::SetXdivPRD: second alocation of XdivPRD not allowed"));
    m_XdivPRD[iDim] = new TFVECT(len); // allocate list of division
    for(int i=0; i<len; i++){
      (*m_XdivPRD[iDim])[i]=xDiv[i]; // set list of division points
    }
  }else
    StopM(std::string("TFOAM::SetXdivPRD: wrong iDim"));
  //[[[[[[[[[[[[[[[[[[[[[
...
// in C++ in format readible for root
  ...
  if(m_OptVert) StopM(std::string("RootPlot2dim:: You can only use this method with OptVert=0"));
...
//-----------------------------------------------------------------------------
  if(m_nDim==2 && m_LastCe<=2000){
    if(m_OptVert) StopM(std::string("LaTexPlot2dim:: You can only use this method with OptVert=0"));
    outfile << "% =========== Vertices Vertices ===========" << endl;
    //Count references of vertices
    int  *NoRefsAc = new int[m_vMax];
    for(iVe = 0 ; iVe<=m_LastVe ; iVe++) NoRefsAc[iVe] = 0;
