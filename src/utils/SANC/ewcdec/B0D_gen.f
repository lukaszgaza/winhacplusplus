      COMPLEX*16 FUNCTION B0D_Wdec(Q2,MU2,M12,M22)
*     ---------------------------------------
* general B0D (subtracted functions); CalcPHEP group
*
      IMPLICIT NONE
      REAL*8 MU2,Q2,RBB,RTB,LNRBB,LNRTB,DREAL,DLOG,DRM12,DRM22
      COMPLEX*16 M12,M22,B0F_Wdec,B0P_Wdec,
     &           B0D1_Wdec,B0D2_Wdec,B0DP_Wdec,B0DW_Wdec,DCMPLX
      RETURN
*
      ENTRY B0D1_Wdec(Q2,MU2,M12,M22)
*     --------------------------
      RTB=-Q2/DREAL(M22)
      IF(RTB.LE.1D-3) THEN
       LNRTB=DLOG(RTB)
       B0D_Wdec=DCMPLX(LNRTB+1D0/2+RTB*(2D0*LNRTB+5D0/3)
     &            +RTB**2*(5D0*LNRTB+59D0/12)
     &            +RTB**3*(14D0*LNRTB+449D0/30)
     &            +RTB**4*(42D0*LNRTB+1417D0/30)
     &            +RTB**5*(132D0*LNRTB+16127D0/105),0D0)
      ELSE
       B0D_Wdec=1D0/RTB*(B0F_Wdec(Q2,MU2,M12,M22)
     &         +DLOG(DREAL(M22)/MU2)-1D0)
      ENDIF
      RETURN
*     
      ENTRY B0D2_Wdec(Q2,MU2,M12,M22)
*     --------------------------
      RTB=-Q2/DREAL(M22)
      IF(RTB.LE.1D-3) THEN
       LNRTB=DLOG(RTB)
       B0D_Wdec=DCMPLX(2D0*LNRTB+5D0/3+RTB*(5D0*LNRTB+59D0/12)
     &            +RTB**2*(14D0*LNRTB+449D0/30)
     &            +RTB**3*(42D0*LNRTB+1417D0/30)
     &            +RTB**4*(132D0*LNRTB+16127D0/105)
     &            +RTB**5*(429D0*LNRTB+429697D0/840),0D0)
      ELSE
       B0D_Wdec=1D0/RTB**2*(B0F_Wdec(Q2,MU2,M12,M22)
     &         +DLOG(DREAL(M22)/MU2)-1D0
     &                 -RTB*(DLOG(DREAL(M12)/DREAL(M22))+1D0/2))
      ENDIF
      RETURN
*
      ENTRY B0DP_Wdec(Q2,MU2,M12,M22)
*     --------------------------
      RTB=-Q2/DREAL(M22)
      IF(RTB.LE.1D-3) THEN
       LNRTB=DLOG(RTB)
       B0D_Wdec=DCMPLX(-LNRTB-17D0/6-RTB*(7D0*LNRTB+133D0/12)
     &            -RTB**2*(31D0*LNRTB+2657D0/60)
     &            -RTB**3*(126D0*LNRTB+5231D0/30)
     &            -RTB**4*(498D0*LNRTB+142991D0/210)
     &            -RTB**5*(1947D0*LNRTB+2225231D0/840),0D0)
      ELSE
       B0D_Wdec=1D0/RTB*((1D0+2D0*RTB)*DREAL(M22)
     &         *B0P_Wdec(Q2,M12,M22)+1D0/2)
      ENDIF
      RETURN
*
      ENTRY B0DW_Wdec(Q2,MU2,M12,M22)
*     --------------------------
      DRM12=DREAL(M12)
      DRM22=DREAL(M22)
      RTB=-Q2/DRM22
      ...
      IF(RTB.LT.1D-3) THEN
       IF(RBB.NE.0D0.AND.RBB.LT.1D-3) THEN
        LNRBB=DLOG(RBB)
        B0D_Wdec=DCMPLX(1D0/2+RBB+RBB**2+RBB**3+RBB**4
     ...
     &  +RTB**4*1D0/30,0D0)
       ELSEIF(RBB.EQ.0D0) THEN
        B0D_Wdec=DCMPLX(1D0/2+1D0/6*RTB+1D0/12*RTB**2+1D0/20*RTB**3
     &                                          +1D0/30*RTB**4,0D0)
     ...
     &          +DLOG(DREAL(M22)/MU2)-1D0) 
      ...
      ELSE
       IF(RBB.NE.0D0) THEN
        B0D_Wdec=1D0/RTB*((1D0-RBB)*(B0F_Wdec(Q2,MU2,M12,M22)
