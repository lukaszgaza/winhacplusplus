...
* But (1.10)^19/(19!)*bernoulli(19)=2.7D-15
      IF (CDABS(1D0-X).LT.1D-13) THEN
        XSPENZ_Wdec=PI*PI/6D0
      ELSE IF (CDABS(1D0-X).LT.0.5D0) THEN
        XSPENZ_Wdec=PI*PI/6D0-CDLOG(1D0-X)*CDLOG(X)-SPENC_Wdec(1D0-X)
      ELSE IF (CDABS(X).GT.1D0) THEN
        XSPENZ_Wdec=-PI*PI/6D0-0.5D0*CDLOG(-X)*CDLOG(-X)
     &              -SPENC_Wdec(1D0/X)
      ELSE
        XSPENZ_Wdec = SPENC_Wdec(X)
      END IF
      END
 
      FUNCTION SPENC_Wdec(X)
      COMPLEX*16 X,SUM,Z,Z2,SPENC_Wdec
      Z=-CDLOG(1D0-X)
      Z2=Z*Z
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C       COMPLEX*16 XSPENZ_Wdec,W,SUM,Z,U
        COMPLEX*16 CSPEN_Wdec ,W,SUM,Z,U
        REAL*8 RZ,AZ,A1
        REAL*8 B(9)
      DATA B/
...
10    W=-CDLOG(1D0-1D0/Z)
      ...
      SUM=SUM+U*B(K)
11    CONTINUE
      CSPEN_Wdec=-SUM-1.64493406684822643D0-.5D0*CDLOG(-Z)**2
      RETURN
20    IF(A1.GT.1D0) GOTO 30
      ...
      SUM=SUM+U*B(K)
21    CONTINUE
      CSPEN_Wdec=-SUM+1.64493406684822643D0-CDLOG(Z)*CDLOG(1D0-Z)
      RETURN
30    W=CDLOG(1D0-1D0/Z)
      ...
      SUM=SUM+U*B(K)
31    CONTINUE
      CSPEN_Wdec=SUM+3.28986813369645287D0
     *               +.5D0*CDLOG(Z-1D0)**2-CDLOG(Z)*CDLOG(1D0-Z)
50    CONTINUE
      RETURN
40    CSPEN_Wdec=DCMPLX(1.64493406684822643D0,0D0)
      RETURN
      END
