      SUBROUTINE SIMPS_Wdec(A1,B1,H1,REPS1,AEPS1,FUNCT,X,AI,AIH,AIABS)
C SIMPS,T,U,V
C SIMPS
  ...
  19  F(K)=10.D16
   ...
   22 IF(C) 2,4,2
    2 RETURN
      ...
      SUBROUTINE SIMPT_Wdec(A1,B1,H1,REPS1,AEPS1,FUNCT,X,AI,AIH,AIABS)
C SIMPT
C A1,B1 -THE LIMITS OF INTEGRATION
C H1 -AN INITIAL STEP OF INTEGRATION
C REPS1,AEPS1 - RELATIVE AND ABSOLUTE PRECISION OF INTEGRATION
  ...
  19  F(K)=10.D16
   ...
   22 IF(C) 2,4,2
    2 RETURN
      ...
      SUBROUTINE SIMPU_Wdec(A1,B1,H1,REPS1,AEPS1,FUNCT,X,AI,AIH,AIABS)
C SIMPU
C A1,B1 -THE LIMITS OF INTEGRATION
C H1 -AN INITIAL STEP OF INTEGRATION
C REPS1,AEPS1 - RELATIVE AND ABSOLUTE PRECISION OF INTEGRATION
  ...
  19  F(K)=10.D16
   ...
   22 IF(C) 2,4,2
    2 RETURN
      ...
      SUBROUTINE SIMPV_Wdec(A1,B1,H1,REPS1,AEPS1,FUNCT,X,AI,AIH,AIABS)
C SIMPV
C A1,B1 -THE LIMITS OF INTEGRATION
C H1 -AN INITIAL STEP OF INTEGRATION
C REPS1,AEPS1 - RELATIVE AND ABSOLUTE PRECISION OF INTEGRATION
  ...
  19  F(K)=10.D16
   ...
   22 IF(C) 2,4,2
    2 RETURN
      ...
      SUBROUTINE FDSIMP_Wdec(AA1,BB1,HH1,REPS1,AEPS1,FUNCT,DFUN,DFUNIN,
     + DUMMY,AI,AIH,AIABS)
*=======================================================================
C        B1
C AI=INT {FUNCT(X)*DDFUN(X)Y}
...
*
      IF(X)8,1,1
1     IF(X-.5D0)2,2,3
2     SPENCE_Wdec=FSPENS_Wdec(X)
      RETURN
3     IF(X-1.D0)4,4,5
4     SPENCE_Wdec=F1-LOG(X)*LOG(1D0-X+1D-15)-FSPENS_Wdec(1D0-X)
      RETURN
5     IF(X-2.D0)6,6,7
6     SPENCE_Wdec=F1-.5D0*LOG(X)*LOG((X-1D0)**2/X)
     &           +FSPENS_Wdec(1D0-1D0/X)
      RETURN
7     SPENCE_Wdec=2D0*F1-.5D0*LOG(X)**2-FSPENS_Wdec(1D0/X)
      RETURN
8     IF(X+1.D0)10,9,9
9     SPENCE_Wdec=-.5D0*LOG(1D0-X)**2-FSPENS_Wdec(X/(X-1D0))
      RETURN
10    SPENCE_Wdec=-.5D0*LOG(1D0-X)*LOG(X**2/(1D0-X))
     &            -F1+FSPENS_Wdec(1D0/(1D0-X))
      END
 
      FUNCTION FSPENS_Wdec(X)
*
      IMPLICIT REAL*8(A-H,O-Z)
*
      A=1D0
...
2     FSPENS_Wdec=F
      ...
      DOUBLE PRECISION FUNCTION DDILOG_Wdec(X)
*=======================================================================
...
c     IF(X.LT.0D0) THEN
c     X1=1D0-X
c     DS12=-TRILOG_Wdec(1D0/X1)-LOG(X1)**2*LOG(-X)/2D0+LOG(X1)**3/6D0
...
c           LI3=-HELP+DLOG(X)*DILOG(X)+0.5D0*DLOG(1D0-X)*DLOG(X)**2+Z3
            TRILOG_Wdec=-HELP+DLOG(X)*DDILOG_Wdec(X)+0.5D0*DLOG(1D0-X)
     &                 *DLOG(X)**2+Z3
         ELSE
          TRILOG_Wdec=Z3
         ENDIF
      ...
      DOUBLE PRECISION FUNCTION TRIS12_Wdec(X)
C Tsuyoshi Matsuura 1987
C
...
C
      ...
      IF (X.LT.0.5D0) THEN
         IF (X.GT.0.0D0) THEN
            ...
            DO 10 I=8,0,-1
               HELP=HELP*Z+S(I)
   10       CONTINUE
            TRIS12_Wdec=1.0D0/2.0D0*(Z*HELP-Z*U/6.0D0)
         ELSE
            TRIS12_Wdec=0.0D0
         ENDIF
      ELSE
   ...
   20       CONTINUE
            HELP=U*HELP
            TRIS12_Wdec=-HELP+DLOG(1.0D0-X)*DDILOG_Wdec(1.0D0-X)+
     +           0.5D0*DLOG(X)*DLOG(1.0D0-X)**2+Z3
      ...
      FUNCTION DLI2_Wdec(X)
      IMPLICIT REAL*8(A-Z)
      DLI2_Wdec=DDILOG_Wdec(X)
      END
 
      FUNCTION DLI3_Wdec(X)
* This is Li3(x) for x<=1 only!
      IMPLICIT REAL*8(A-Z)
      DATA ZET2/1.6449340668482262D0/
      DATA ZET3/1.2020569031595943D0/
      IF(X.GT.0D0) THEN
        DLI3_Wdec=TRILOG_Wdec(X)
      ...
      ELSE IF(X.GT.-1D0) THEN
        ...
        DLOGX1=DLOG(X1)
        XX=-X/X1
        DLI3_Wdec=+TRIS12_Wdec(XX)-TRILOG_Wdec(XX)
     &       +DLOGX1*(DDILOG_Wdec(X)+1D0/3*DLOGX1*DLOGX1)
      ELSE IF(X.EQ.-1D0) THEN
        DLI3_Wdec=-3D0/4*ZET3
      ELSE
        X1=1D0-X
        DLOGX1=DLOG(X1)
        DLOGXM=DLOG(-X)
        DLI3_Wdec=+TRIS12_Wdec(1D0/X1)-TRILOG_Wdec(1D0/X1)
     &       +DLOGX1*DLOGX1*(-DLOGXM/2D0+DLOGX1/3D0)
     &       +(DLOGXM-DLOGX1)*DDILOG_Wdec(X)-ZET2*DLOGX1
     &       -DLOGX1*DLOGXM*(DLOGX1-DLOGXM)/2D0
      ENDIF
      END
 
      FUNCTION DS12_Wdec(X)
* This is S12(x) for x<=1 only!
      IMPLICIT REAL*8(A-Z)
      DATA ZET2/1.6449340668482262D0/
      DATA ZET3/1.2020569031595943D0/
      IF(X.GT.0D0) THEN
        DS12_Wdec=TRIS12_Wdec(X)
      ELSE IF(X.EQ.0D0) THEN
        DS12_Wdec=0D0
      ELSE IF(X.GT.-1D0) THEN
        X1=1D0-X
        DLOGX1=DLOG(X1)
        DS12_Wdec=TRIS12_Wdec(-X/X1)+1D0/6*DLOGX1**3
      ELSE IF(X.EQ.-1D0) THEN
        DS12_Wdec=ZET3/8
      ELSE
        X1=1D0-X
        DLOGX1=DLOG(X1)
        DS12_Wdec=-TRILOG_Wdec(1D0/X1)
     &       -DLOGX1*DLOGX1*(DLOG(-X)/2D0-DLOGX1/6D0)
     &       -DLOGX1*DDILOG_Wdec(X)+ZET3-ZET2*DLOGX1
      ENDIF
      END
